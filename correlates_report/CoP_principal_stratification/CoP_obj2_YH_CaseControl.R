...
S1[Z==0]<-NA
S1[is.na(dat.wide.ppim$wt.ppimmfl)]<-NA
W[is.na(dat.wide.ppim$wt.ppimmfl)]<-NA
...
VE1<-integVE(Su,Wu=Wu,datin.nomis,varlist=NULL,beta=fit1)
...
VE2<-integVE(Su,Wu=Wu,datin.nomis,varlist=c("W2","W3","W4"),beta=fit2)


### With covariate adjustment, baseline marker not affecting risk
X<-dat.wide.ppim$vacc2birthLESSTHAN30*1000+dat.wide.ppim$riskScoreBin.mat.endpoint1*100+dat.wide.ppim$group
Xu<-unique(sort(X))
...
WX<-dat.wide.ppim$vacc2birthLESSTHAN30*100+dat.wide.ppim$riskScoreBin.mat.endpoint1*10+as.numeric(cut(dat.wide.ppim[,bsm],c(-Inf,qq,Inf)))
WXu<-unique(sort(WX))

datin.nomis$W=datin.nomis$VB*100+datin.nomis$RS*10+datin.nomis$W

VE3<-integVE(Su,Wu=WXu,datin.nomis,varlist=c("VB","RS"),beta=fit3)
...
VE4<-integVE(Su,Wu=WXu,datin.nomis,varlist=c("W2","W3","W4","VB","RS"),beta=fit4)
