...
#' @export
bootRiskCurve <- function(formula, bsm, tx, data, pstype=c("continuous", "ordered"), bsmtype=c("continuous", "ordered"), 
                          bwtype=c("fixed", "generalized_nn", "adaptive_nn"), hinge=FALSE, weights=NULL, psGrid=NULL, iter, 
                          seed=NULL, saveFile=NULL, saveDir=NULL){
  ...
  # EIA and PCA readouts are available for all subjects; therefore, data=data2
  ...
  bRiskCurveList <- lapply(1:iter, function(i, formulaDecomp, Sterm, Sbterm){
    ...
    # EIA and PCA readouts are available for all subjects
    bdata2 <- subset(bdata, !is.na(S))
    
    # this is the phase 2 sample
    bdata3 <- subset(bdata, !is.na(weights))
    ...
    if (hinge){
      ...
      fit1 <- tps(fm, data=subset(bdata3, Z==0 & !is.na(Y)), nn0=NROW(subset(bdata, Z==0 & Y==0)), nn1=NROW(subset(bdata, Z==0 & Y==1)), group=rep(1, NROW(subset(bdata3, Z==0 & !is.na(Y)))), method="PL", cohort=TRUE)
    } else {
    ...
    curves <- lapply(psGrid, function(s){ risk(s, fm, bdata, bdata3, pstype, bsmtype, fit1, fhat, ghat, cpoint) })
...
riskCurve <- function(formula, bsm, tx, data, pstype=c("continuous", "ordered"), bsmtype=c("continuous", "ordered"), bwtype=c("fixed", "generalized_nn", "adaptive_nn"),
  ...
  # this is the phase 2 sample
  data3 <- subset(data, !is.na(weights))
  ...
  if (hinge){
    ...
    fit1 <- tps(fm, data=subset(data3, Z==0 & !is.na(Y)), nn0=nPControls, nn1=nPCases, group=rep(1, NROW(subset(data3, Z==0 & !is.na(Y)))), method="PL", cohort=TRUE)
  } else {
    # for passing on to the function 'risk'
    cpoint <- NULL
  ...
  if (is.null(psGrid)){
    psGrid <- seq(min(data2$S[data2$Z==1]), max(data2$S[data2$Z==1]), length.out=200)
  }
  ...
  curves <- lapply(psGrid, function(s){ risk(s, fm, data, data3, pstype, bsmtype, fit1, fhat, ghat, cpoint) })
  plaRiskCurve <- sapply(curves, "[[", "plaRisk")
  txRiskCurve <- sapply(curves, "[[", "txRisk")
...
#' @export
plotMCEPcurve <- function(object, confLevel=0.95, hingePoint=NULL, title=NULL, xLab=NULL, xTicks=NULL, yLab=NULL, 
                          xLim=NULL, yLim=NULL, pType=c("l","p"), CE=NULL){
  ...
  object[, -1] <- ifelse(object[, -1] %in% c(-Inf, Inf), NA, object[, -1])
  
  par(mar=c(5,5,4,2), cex.lab=cexLab, cex.axis=cexAxis, las=1)
  ...
  if (sum(!is.na(object[, -1]))==0){
    plot(object[, 1], object[, 2], type="n", ylim=0:1, xlab=xLab, ylab=yLab, xaxt=ifelse(is.null(xTicks), "s", "n"))
    if (!is.null(xTicks)){ 
      axis(side=1, at=log10(xTicks[seq(1, length(xTicks), by=2)]), labels=xTicks[seq(1, length(xTicks), by=2)], cex.axis=cexAxis) 
      axis(side=1, at=log10(xTicks[seq(2, length(xTicks), by=2)]), labels=xTicks[seq(2, length(xTicks), by=2)], cex.axis=cexAxis)
    }
  } else {
    if (is.null(yLim)){ 
      yLim <- c(-0.2, 1)
    ...
    if (is.null(xLim)){
      plot(object[, 1], object[, 2], type="n", ylim=yLim, xlab=xLab, ylab=yLab, xaxt=ifelse(is.null(xTicks), "s", "n"), yaxt="n")  
    } else {
      plot(object[, 1], object[, 2], type="n", xlim=xLim, ylim=yLim, xlab=xLab, ylab=yLab, xaxt=ifelse(is.null(xTicks), "s", "n"), yaxt="n")
    }
    
    if (!is.null(xTicks)){ 
      axis(side=1, at=log10(xTicks[seq(1, length(xTicks), by=2)]), labels=xTicks[seq(1, length(xTicks), by=2)], cex.axis=cexAxis) 
      axis(side=1, at=log10(xTicks[seq(2, length(xTicks), by=2)]), labels=xTicks[seq(2, length(xTicks), by=2)], cex.axis=cexAxis)
      axis(side=2, at=seq(-0.2, 1, by=0.2), cex.axis=cexAxis)
    }
    if (!is.null(title)){ mtext(title, side=3, line=0.5, cex=cexTitle) }
    abline(h=ifelse(colnames(object)[2]=="rr", 1, 0), lty="dotted", lwd=2, col="gray70")
    
    if (!is.null(CE)){ abline(h=CE, lty="dotted", lwd=3, col="red")}
