...
plot_assays <- function(avgs, type = "auc", main_font_size_forest,
  ...
  top_learner_plot <- top_learners %>%
    ggplot(aes(x = measure, y = factor(paste0(Screen, "_", Learner), levels = paste0(Screen, "_", Learner)[order(measure)],
                                       labels = paste0(learner_nm, " ", screen_nm)[order(measure)]))) +
  ...
  top_learners_labels <- top_learners %>%
    ...
    select(learner_nm, screen_nm, lab_measure)
  ...
  top_learners_labs$x_coord <- apply(matrix(top_learners_labs$variable), 1, function(x) which(grepl(x, c("learner_nm", "screen_nm", "lab_measure"))) - 1 + c(0, -0.3, 0.2)[which(grepl(x, c("learner_nm", "screen_nm", "lab_measure")))])
