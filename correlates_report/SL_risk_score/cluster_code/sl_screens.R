...
## SL screens
## -------------------------------------------------------------------------------------
# rank variables based on univariate logistic regression p value
...
# no screen
screen_all_plus_exposure<- function(Y, X, family, obsWeights, id, nVar=4,...){
  #X contain baseline variables
  ## logistic regression of outcome on each variable
...
# screen based on p minimim p value of univariate logistic regression
# screen_univariate_logistic_pval_plus_exposure  <- function(Y, X, family, obsWeights, id, minPvalue = 0.1, nVar = 4, ...) {
#   ## logistic regression of outcome on each variable
#   listp <- apply(X, 2, function(x, Y, family) {
#     summ <- coef(summary(glm(Y ~ x , family = family, weights = obsWeights)))
#     ifelse(dim(summ)[1] > 1, summ[2, 4], 1)
...
#   
#   # keep only a max of nVar immune markers; rank by univariate p-value in a model 
#   
...
screen_highcor_random_plus_exposure <- function(Y, X, family, obsWeights, id, nVar = 4, ...) {
  ...
  sampCols <- sample(names(vars[vars]), nVar)
