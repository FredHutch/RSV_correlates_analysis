...
thresholdTMLE <- function(data_full, node_list, thresholds = NULL, biased_sampling_strata = NULL, biased_sampling_indicator = NULL, lrnr_A = Lrnr_glmnet$new(), lrnr_Y = Lrnr_glmnet$new(), lrnr_Delta = Lrnr_glmnet$new()) {
  ...
  if(!is.null(biased_sampling_indicator)) {
    ...
    data_full$grp <- as.character(data_full[[biased_sampling_strata]])
...
plot_threshold_response <- function(output, simultaneous_CI = T, monotone = F) {
  ...
  if(monotone) {
    subtitle <- paste0("Assumes monotonicity (nonincreasing) of true function")
  ...
  if(!is.null(estimates)) {
    ...
    g1 <- g1 + ggtitle(title )  +labs(subtitle = subtitle) +theme(plot.title = element_text(size=12), plot.subtitle = element_text(size=10))
