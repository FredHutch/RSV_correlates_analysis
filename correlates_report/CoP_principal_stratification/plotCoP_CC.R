...
load(file=paste0("input/Result/outCC_",ps,"_",y.v,".Rdata"))
load(file=paste0("input/Result/outbootCC_",ps,"_",y.v,".Rdata"))
...
mypdf(file=paste0("input/figCC_nocov",ps.seq[index1],'_',y.seq[index2]),width=6,height=6)
...
mypdf(file=paste0("input/figCC_withcov",ps.seq[index1],'_',y.seq[index2]),width=6,height=6)
plot(Su,VE.S,type='l',ylim=c(-1,1),xlab=quote(s[1]),ylab=quote(VE(s[1])),main=paste0(ps.name[index1],',',y.name[index2]),xaxt="n", yaxt="n", col="red3", lwd=2.5)
lines(Su,VE.S,col=2,lwd=lwd)
lines(Su,low.ci,lty=2,lwd=lwd)
lines(Su,high.ci,lty=2,lwd=lwd)
